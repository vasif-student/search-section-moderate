...
namespace RentCar.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> SearchCar(CarSearchViewModel model)
        {
            ...
            {
                ViewBag.Error = "Başlanğıc tarix son tarixdən böyük ola bilməz";
            }
            else
            {
                if(carModels == null)
                {
                    ViewBag.NoResult = "Avtomobil tapilmadi";
                }
                else
                {
                    foreach (var carModel in carModels)
                    {
                        foreach (var rentedCar in rentedCars)
                        {
                            ...
                            {
                                if ((model.StartDate < rentedCar.StartDate && model.EndDate < rentedCar.StartDate)
                                    || (model.StartDate > rentedCar.EndDate && model.EndDate > rentedCar.EndDate))
                                {
